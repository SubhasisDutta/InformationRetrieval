...
public class MakeInformationSchema {
	public static void initilize() {
		String location = CommonUtils.DB_LOCATION;
		try {
			/* DONE: Put all binary data files in a separate subdirectory (subdirectory tree?) */
			/* Use Hard Coded: Should there not be separate Class static variables for the file names? 
			 *        and just hard code them here?
			 */
			/* Check in Main: Should there be separate methods to check for and subsequently create each file 
			 *       granularly, instead of a big bang all or nothing? 
			 *       - Check if all 3 files are present if not present rebuild all files. 
			 *       As doing single file creation can lead to data in consistency. 
			 */
			RandomAccessFile schemataTableFile = new RandomAccessFile(location+InformationSchemaUtility.INFORMATION_SCHEMA_SCHEMATA, "rw");
			RandomAccessFile tablesTableFile = new RandomAccessFile(location+InformationSchemaUtility.INFORMATION_SCHEMA_TABLE, "rw");
			RandomAccessFile columnsTableFile = new RandomAccessFile(location+InformationSchemaUtility.INFORMATION_SCHEMA_COLUMNS, "rw");
			...
			System.out.println("The database has been initilized and is stored in location "+ location);
		}
		catch(Exception e) {
